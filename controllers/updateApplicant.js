...
const applicantId = req.params.id; 
  ...
  try {
    ...
    const {
      ...
      state, city, postal_code, current_designation, total_experience_years
    } = req.body;
    ...
    if (req.body.education) {
      for (const edu of Object.values(req.body.education)) {
        if (edu.course_type_id && edu.board_or_uni && edu.year) {
          await conn.query(
            `INSERT INTO applicant_educations 
             (applicant_id, course_type_id, board_or_university, passing_year, percentage) 
             VALUES (?, ?, ?, ?, ?)`,
            [applicantId, edu.course_type_id, edu.board_or_uni, edu.year, edu.percentage || null]
          );
        }
      }
    }
    ...
    // =========================
    if (req.body.work) {
      for (const w of Object.values(req.body.work)) {
        if (w.company_name && w.designation && w.start_date) {
          await conn.query(
            `INSERT INTO applicant_work_experiences 
             (applicant_id, company_name, designation, start_date, end_date) 
             VALUES (?, ?, ?, ?, ?)`,
            [applicantId, w.company_name, w.designation, w.start_date, w.end_date || null]
          );
        }
      }
    }
    ...
    if (req.body.languages) {
      for (const lang of Object.values(req.body.languages)) {
        if (lang.language_id) {
          await conn.query(
    ...
    if (req.body.technologies) {
      for (const tech of Object.values(req.body.technologies)) {
        if (tech.technology_id && tech.proficiency_level_id) {
          await conn.query(
    ...
    if (req.body.references) {
      for (const ref of Object.values(req.body.references)) {
        if (ref.name && ref.relationship_id) {
          await conn.query(
